---
title: "Guía de Extensión y Creación de Plugins: tyrell"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de tyrell"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-tyrell-plugins)=
# Guía de Extensión y Plugins: tyrell

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`tyrell`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-tyrell-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`tyrell` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para tyrell`.
- **Punto de Entrada Oficial**: `tyrell.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-tyrell-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para tyrell."""
    name = "custom_rule"
...
Para que `tyrell` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."tyrell.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `tyrell` la reconozca:
...
(manual-tyrell-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `tyrell` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = tyrell.ejecutar_analisis(
...
(manual-tyrell-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `tyrell` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar tyrell
        run: pip install -e ./tyrell
        
      - name: Ejecutar Auditoría
        run: tyrell check src/ include/ --json > reporte.json
````

---

(manual-tyrell-plugins-ejercicios)=
